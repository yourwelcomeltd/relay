...
var GraphQLRange = function () {
  ...
  GraphQLRange.prototype.addItems = function addItems(queryCalls, edges, pageInfo) {
    ...
    if (calls.first) {
      ...
      if (calls.before && !calls.after) {
        ...
        if (pageInfo[require('./RelayConnectionInterface').HAS_NEXT_PAGE] === true && edges.length !== 0) {
          ...
          this._addAfterFirstItems(edges, pageInfo[require('./RelayConnectionInterface').HAS_NEXT_PAGE], undefined, calls.before);
        } else {
          // Since there is no gap, we can stitch into the beginning
          // of existing segment
          this._addBeforeLastItems(edges, pageInfo[require('./RelayConnectionInterface').HAS_PREV_PAGE], calls.before);
        }
      } else {
        ...
        this._addAfterFirstItems(edges, pageInfo[require('./RelayConnectionInterface').HAS_NEXT_PAGE], calls.after, calls.before);
      }
    } else if (calls.last) {
      // after().last() calls can produce gaps
      if (calls.after && !calls.before) {
        // make a new segment if there is a gap and there are new edges
        if (pageInfo[require('./RelayConnectionInterface').HAS_PREV_PAGE] === true && edges.length !== 0) {
          ...
          // just storing a last().
          this._addBeforeLastItems(edges, pageInfo[require('./RelayConnectionInterface').HAS_PREV_PAGE], undefined, calls.after);
        } else {
          // Since there is no gap, we can stitch to the end
          // of existing segment
          this._addAfterFirstItems(edges, pageInfo[require('./RelayConnectionInterface').HAS_NEXT_PAGE], calls.after);
        }
      } else {
        // These elements are added from paging to extend the the range.
        if (!calls.before) {
        ...
        this._addBeforeLastItems(edges, pageInfo[require('./RelayConnectionInterface').HAS_PREV_PAGE], calls.before, calls.after);
      }
    }
  };
  ...
  GraphQLRange.prototype._retrieveRangeInfoForStaticCalls = function _retrieveRangeInfoForStaticCalls(queryCalls) {
    ...
    if (storedInfo) {
      ...
      return {
        ...
        diffCalls: [],
        pageInfo: (_pageInfo = {}, (0, _defineProperty3['default'])(_pageInfo, require('./RelayConnectionInterface').START_CURSOR, storedInfo.cursors[0]), (0, _defineProperty3['default'])(_pageInfo, require('./RelayConnectionInterface').END_CURSOR, storedInfo.cursors[storedInfo.cursors.length - 1]), (0, _defineProperty3['default'])(_pageInfo, require('./RelayConnectionInterface').HAS_NEXT_PAGE, true), (0, _defineProperty3['default'])(_pageInfo, require('./RelayConnectionInterface').HAS_PREV_PAGE, true), _pageInfo)
      };
    }
  ...
  GraphQLRange.prototype._retrieveRangeInfoForFirstQuery = function _retrieveRangeInfoForFirstQuery(queryCalls, queuedRecord) {
    ...
    if (requestedCursors.length) {
      pageInfo[require('./RelayConnectionInterface').START_CURSOR] = requestedCursors[0];
      pageInfo[require('./RelayConnectionInterface').END_CURSOR] = requestedCursors[requestedCursors.length - 1];
    }
    ...
    if (!this._hasLast || segmentIndex !== this._orderedSegments.length - 1 || lastID && lastID !== segment.getLastID()) {
      pageInfo[require('./RelayConnectionInterface').HAS_NEXT_PAGE] = true;
    ...
    if (queuedRecord) {
      ...
      if (appendEdgeIDs && appendEdgeIDs.length && !pageInfo[require('./RelayConnectionInterface').HAS_NEXT_PAGE]) {
  ...
  GraphQLRange.prototype._retrieveRangeInfoForLastQuery = function _retrieveRangeInfoForLastQuery(queryCalls, queuedRecord) {
    ...
    if (!this._hasFirst || segmentIndex !== 0 || firstID && firstID !== segment.getFirstID()) {
      pageInfo[require('./RelayConnectionInterface').HAS_PREV_PAGE] = true;
      if (requestedEdgeIDs.length < countNeeded) {
        countNeeded -= requestedEdgeIDs.length;
    ...
    if (queuedRecord) {
      ...
      if (prependEdgeIDs && prependEdgeIDs.length && !pageInfo[require('./RelayConnectionInterface').HAS_PREV_PAGE]) {
        requestedEdgeIDs = prependEdgeIDs.concat(requestedEdgeIDs);
      }
      if (removeIDs && removeIDs.length) {
        requestedEdgeIDs = requestedEdgeIDs.filter(function (edgeID) {

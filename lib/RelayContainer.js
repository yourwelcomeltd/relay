...
function createContainerComponent(Component, spec) {
  var ComponentClass = require('./RelayContainerUtils').getReactComponent(Component);
  var componentName = require('./RelayContainerUtils').getComponentName(Component);
  ...
  var RelayContainer = function (_React$Component) {
    ...
    function RelayContainer(props, context) {
      ...
      !require('./isRelayEnvironment')(relay) ? process.env.NODE_ENV !== 'production' ? require('fbjs/lib/invariant')(false, 'RelayContainer: `%s` was rendered with invalid Relay context `%s`. ' + 'Make sure the `relay` property on the React context conforms to the ' + '`RelayEnvironment` interface.', containerName, relay) : require('fbjs/lib/invariant')(false) : void 0;
    ...
    RelayContainer.prototype.componentWillReceiveProps = function componentWillReceiveProps(nextProps, maybeNextContext) {
      ...
      !nextContext ? process.env.NODE_ENV !== 'production' ? require('fbjs/lib/invariant')(false, 'RelayContainer: Expected a context to be set.') : require('fbjs/lib/invariant')(false) : void 0;
    ...
    RelayContainer.prototype._updateFragmentPointers = function _updateFragmentPointers(props, context, variables, prevVariables) {
      ...
      fragmentNames.forEach(function (fragmentName) {
        ...
        if (fragment.isPlural()) {
          ...
          if (_dataIDs2) {
            !(_dataIDs2.length === propValue.length) ? process.env.NODE_ENV !== 'production' ? require('fbjs/lib/invariant')(false, 'RelayContainer: Invalid prop `%s` supplied to `%s`. Some ' + 'array items contain data fetched by Relay and some items ' + 'contain null/mock data.', fragmentName, componentName) : require('fbjs/lib/invariant')(false) : void 0;
          }
...
function getContainerName(Component) {
  return 'Relay(' + require('./RelayContainerUtils').getComponentName(Component) + ')';
}

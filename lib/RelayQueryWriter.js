...
var RelayQueryWriter = function (_RelayQueryVisitor) {
  ...
  RelayQueryWriter.prototype.createRecordIfMissing = function createRecordIfMissing(node, recordID, path, payload) {
    ...
    if (recordState !== require('./RelayRecordState').EXISTENT) {
  ...
  RelayQueryWriter.prototype.visitField = function visitField(field, state) {
    ...
    !(this._writer.getRecordState(recordID) === require('./RelayRecordState').EXISTENT) ? process.env.NODE_ENV !== 'production' ? require('fbjs/lib/invariant')(false, 'RelayQueryWriter: Cannot update a non-existent record, `%s`.', recordID) : require('fbjs/lib/invariant')(false) : void 0;
  ...
  RelayQueryWriter.prototype._writeConnection = function _writeConnection(field, state, recordID, connectionData) {
    ...
    var hasEdges = !!(field.getFieldByStorageKey(require('./RelayConnectionInterface').EDGES) || connectionData != null && typeof connectionData === 'object' && connectionData[require('./RelayConnectionInterface').EDGES]);
    ...
    if (connectionRecordState !== require('./RelayRecordState').EXISTENT) {
      this.recordUpdate(recordID);
  ...
  state) {
    ...
    node.getChildren().forEach(function (child) {
      if (child instanceof require('./RelayQuery').Field) {
        if (child.getSchemaName() === require('./RelayConnectionInterface').EDGES) {
          _this3._writeEdges(connection, child, state);
        } else if (child.getSchemaName() !== require('./RelayConnectionInterface').PAGE_INFO) {
  ...
  RelayQueryWriter.prototype._writeEdges = function _writeEdges(connection, edges, state) {
    ...
    var edgesData = connectionData[require('./RelayConnectionInterface').EDGES];
    ...
    !require('./RelayConnectionInterface').hasRangeCalls(rangeCalls) ? process.env.NODE_ENV !== 'production' ? require('fbjs/lib/invariant')(false, 'RelayQueryWriter: Cannot write edges for connection on record ' + '`%s` without `first`, `last`, or `find` argument.', connectionID) : require('fbjs/lib/invariant')(false) : void 0;
    ...
    edgesData.forEach(function (edgeData) {
      ...
      var nodeData = edgeData[require('./RelayConnectionInterface').NODE];
      if (nodeData == null) {
        return;
      }

      !(typeof nodeData === 'object') ? process.env.NODE_ENV !== 'production' ? require('fbjs/lib/invariant')(false, 'RelayQueryWriter: Expected node to be an object for field `%s` on ' + 'record `%s`.', connection.getDebugName(), connectionID) : require('fbjs/lib/invariant')(false) : void 0;
      ...
      var nodeID = nodeData && nodeData[require('./RelayNodeInterface').ID] || prevEdge && _this4._store.getLinkedRecordID(prevEdge.edgeID, require('./RelayConnectionInterface').NODE) || require('./generateClientID')();
    ...
    var pageInfo = connectionData[require('./RelayConnectionInterface').PAGE_INFO] || require('./RelayConnectionInterface').getDefaultPageInfo();
  ...
  RelayQueryWriter.prototype._writePluralLink = function _writePluralLink(field, state, recordID, fieldData) {
    ...
    fieldData.forEach(function (nextRecord) {
      ...
      !(typeof nextRecord === 'object' && nextRecord) ? process.env.NODE_ENV !== 'production' ? require('fbjs/lib/invariant')(false, 'RelayQueryWriter: Expected elements for plural field `%s` to be ' + 'objects.', storageKey) : require('fbjs/lib/invariant')(false) : void 0;
  ...
  RelayQueryWriter.prototype._writeLink = function _writeLink(field, state, recordID, fieldData) {
    ...
    var storageKey = field.getStorageKey();
    !(typeof fieldData === 'object' && fieldData !== null) ? process.env.NODE_ENV !== 'production' ? require('fbjs/lib/invariant')(false, 'RelayQueryWriter: Expected data for non-scalar field `%s` on record ' + '`%s` to be an object.', field.getDebugName(), recordID) : require('fbjs/lib/invariant')(false) : void 0;
    ...
    var nextLinkedID = field.getSchemaName() === require('./RelayConnectionInterface').NODE && nodeID || fieldData[require('./RelayNodeInterface').ID] || prevLinkedID || require('./generateClientID')();

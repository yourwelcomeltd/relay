...
var RelayStoreReader = function (_RelayQueryVisitor) {
  ...
  RelayStoreReader.prototype.visitField = function visitField(node, state) {
    ...
    if (rangeInfo && node.getSchemaName() === require('./RelayConnectionInterface').EDGES) {
      this._readEdges(node, rangeInfo, state);
    } else if (rangeInfo && node.getSchemaName() === require('./RelayConnectionInterface').PAGE_INFO) {
  ...
  RelayStoreReader.prototype._readEdges = function _readEdges(node, rangeInfo, state) {
    ...
    var previousData = getDataValue(state, require('./RelayConnectionInterface').EDGES);
    ...
    this._setDataValue(state, require('./RelayConnectionInterface').EDGES, edges);
  };
  ...
  RelayStoreReader.prototype._readPageInfo = function _readPageInfo(node, rangeInfo, state) {
    ...
    !pageInfo ? process.env.NODE_ENV !== 'production' ? require('fbjs/lib/invariant')(false, 'readRelayQueryData(): Missing field, `%s`.', require('./RelayConnectionInterface').PAGE_INFO) : require('fbjs/lib/invariant')(false) : void 0;
    if (rangeInfo.diffCalls.length) {
      state.isPartial = true;
    }
    ...
    this._setDataValue(state, require('./RelayConnectionInterface').PAGE_INFO, nextData);
  };
...
var RelayRangeCallEnforcer = function (_RelayQueryVisitor2) {
  ...
  RelayRangeCallEnforcer.prototype.visitField = function visitField(node, parent) {
    ...
    !(schemaName !== require('./RelayConnectionInterface').EDGES && schemaName !== require('./RelayConnectionInterface').PAGE_INFO) ? process.env.NODE_ENV !== 'production' ? require('fbjs/lib/invariant')(false, 'readRelayQueryData(): The field `%s` is a connection. Fields `%s` and ' + '`%s` cannot be fetched without a `first`, `last` or `find` argument.', parent.getApplicationName(), require('./RelayConnectionInterface').EDGES, require('./RelayConnectionInterface').PAGE_INFO) : require('fbjs/lib/invariant')(false) : void 0;
  };

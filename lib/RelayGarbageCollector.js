...
var RelayGarbageCollector = function () {
  ...
  RelayGarbageCollector.prototype.decrementReferenceCount = function decrementReferenceCount(dataID) {
    if (!this._refCounts.hasOwnProperty(dataID) || this._refCounts[dataID] <= 0) {
      process.env.NODE_ENV !== 'production' ? require('fbjs/lib/warning')(false, 'RelayGarbageCollector: Expected id `%s` be referenced before being ' + 'unreferenced.', dataID) : void 0;
      this._refCounts[dataID] = 0;
      return;
    }
    this._refCounts[dataID]--;
  ...
  RelayGarbageCollector.prototype.acquireHold = function acquireHold() {
    ...
    return {
      release: function release() {
        !!isReleased ? process.env.NODE_ENV !== 'production' ? require('fbjs/lib/invariant')(false, 'RelayGarbageCollector: hold can only be released once.') : require('fbjs/lib/invariant')(false) : void 0;
        !(_this._activeHoldCount > 0) ? process.env.NODE_ENV !== 'production' ? require('fbjs/lib/invariant')(false, 'RelayGarbageCollector: cannot decrease hold count below zero.') : require('fbjs/lib/invariant')(false) : void 0;
        isReleased = true;

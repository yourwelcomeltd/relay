...
function printRelayOSSQuery(node) {
  ...
  !queryText ? process.env.NODE_ENV !== 'production' ? require('fbjs/lib/invariant')(false, 'printRelayOSSQuery(): Unsupported node type.') : require('fbjs/lib/invariant')(false) : void 0;
  var variables = {};
...
function printOperation(node, printerState) {
  ...
  !inputString ? process.env.NODE_ENV !== 'production' ? require('fbjs/lib/invariant')(false, 'printRelayOSSQuery(): Expected %s `%s` to have a value for `%s`.', operationKind, node.getName(), node.getCallVariableName()) : require('fbjs/lib/invariant')(false) : void 0;
  // Note: children must be traversed before printing variable definitions
  var children = printChildren(node, printerState, oneIndent);

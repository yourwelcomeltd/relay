...
var RelayGarbageCollection = {
  ...
  initialize: function initialize(stepLength) {
    !(stepLength > 0) ? process.env.NODE_ENV !== 'production' ? require('fbjs/lib/invariant')(false, 'RelayGarbageCollection: step length must be greater than zero, got ' + '`%s`.', stepLength) : require('fbjs/lib/invariant')(false) : void 0;
...
function scheduler(run) {
  ...
  var runIteration = function runIteration() {
    ...
    process.env.NODE_ENV !== 'production' ? require('fbjs/lib/warning')(!pendingQueryTracker.hasPendingQueries(), 'RelayGarbageCollection: GC is executing during a fetch, but the ' + 'pending query may rely on data that is collected.') : void 0;
    var iterations = 0;
    var hasNext = true;

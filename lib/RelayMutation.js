...
var RelayMutation = function () {
  ...
  RelayMutation.getFragment = function getFragment(fragmentName, variableMapping) {
    ...
    if (!fragmentBuilder) {
      !false ? process.env.NODE_ENV !== 'production' ? require('fbjs/lib/invariant')(false, '%s.getFragment(): `%s` is not a valid fragment name. Available ' + 'fragments names: %s', this.name, fragmentName, (0, _keys2['default'])(fragments).map(function (name) {
        return '`' + name + '`';

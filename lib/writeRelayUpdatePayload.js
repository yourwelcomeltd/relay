...
var EDGES_FIELD = require('./RelayQuery').Field.build({
  fieldName: require('./RelayConnectionInterface').EDGES,
...
var IGNORED_KEYS = (0, _defineProperty3['default'])({
  error: true
}, require('./RelayConnectionInterface').CLIENT_MUTATION_ID, true);
...
function deleteRecord(writer, recordID) {
  ...
  if (connectionIDs) {
    connectionIDs.forEach(function (connectionID) {
      ...
      recordWriter.applyRangeUpdate(connectionID, edgeID, require('./GraphQLMutatorConstants').REMOVE);
...
function handleMerge(writer, payload, operation) {
  ...
  for (var fieldName in payload) {
    ...
    if (require('./RelayNodeInterface').ID in payloadData ||
...
function handleRangeAdd(writer, payload, operation, config, isOptimisticUpdate) {
  var clientMutationID = getString(payload, require('./RelayConnectionInterface').CLIENT_MUTATION_ID);
  !clientMutationID ? process.env.NODE_ENV !== 'production' ? require('fbjs/lib/invariant')(false, 'writeRelayUpdatePayload(): Expected operation `%s` to have a `%s`.', operation.getName(), require('./RelayConnectionInterface').CLIENT_MUTATION_ID) : require('fbjs/lib/invariant')(false) : void 0;
  var store = writer.getRecordStore();
...
function addRangeNode(writer, operation, config, connectionID, nodeID, edgeData) {
  ...
  if (rangeBehavior === require('./GraphQLMutatorConstants').IGNORE) {
    return;
  }
  ...
  !hasEdgeField ? process.env.NODE_ENV !== 'production' ? require('fbjs/lib/invariant')(false, 'writeRelayUpdatePayload(): Expected mutation query to include the ' + 'relevant edge field, `%s`.', config.edgeName) : require('fbjs/lib/invariant')(false) : void 0;
  ...
  if (rangeBehavior in require('./GraphQLMutatorConstants').RANGE_OPERATIONS) {
    ...
    console.error('writeRelayUpdatePayload(): invalid range operation `%s`, valid ' + 'options are `%s`, `%s`, `%s`, or `%s`.', rangeBehavior, require('./GraphQLMutatorConstants').APPEND, require('./GraphQLMutatorConstants').PREPEND, require('./GraphQLMutatorConstants').IGNORE, require('./GraphQLMutatorConstants').REFETCH);
  }
}
...
function handleRangeDelete(writer, payload, config) {
  ...
  if (Array.isArray(config.deletedIDFieldName)) {
    ...
    !(recordIDs == null || !Array.isArray(recordIDs) || typeof recordIDs !== 'string') ? process.env.NODE_ENV !== 'production' ? require('fbjs/lib/invariant')(false, 'writeRelayUpdatePayload(): Expected `%s` to be an array/string, got `%s`.', config.deletedIDFieldName, (0, _stringify2['default'])(recordIDs)) : require('fbjs/lib/invariant')(false) : void 0;
  ...
  !(recordIDs != null) ? process.env.NODE_ENV !== 'production' ? require('fbjs/lib/invariant')(false, 'writeRelayUpdatePayload(): Missing ID(s) for deleted record at field `%s`.', config.deletedIDFieldName) : require('fbjs/lib/invariant')(false) : void 0;
...
function getString(payload, field) {
  ...
  !(value == null || typeof value === 'string') ? process.env.NODE_ENV !== 'production' ? require('fbjs/lib/invariant')(false, 'writeRelayUpdatePayload(): Expected `%s` to be a string, got `%s`.', field, (0, _stringify2['default'])(value)) : require('fbjs/lib/invariant')(false) : void 0;
  return value;
}

function getObject(payload, field) {
  var value = payload[field];
  !(value == null || typeof value === 'object' && !Array.isArray(value)) ? process.env.NODE_ENV !== 'production' ? require('fbjs/lib/invariant')(false, 'writeRelayUpdatePayload(): Expected `%s` to be an object, got `%s`.', field, (0, _stringify2['default'])(value)) : require('fbjs/lib/invariant')(false) : void 0;
  return value;
}

function getObjectOrArray(payload, field) {
  var value = payload[field];
  !(value == null || typeof value === 'object') ? process.env.NODE_ENV !== 'production' ? require('fbjs/lib/invariant')(false, 'writeRelayUpdatePayload(): Expected `%s` to be an object/array, got `%s`.', field, (0, _stringify2['default'])(value)) : require('fbjs/lib/invariant')(false) : void 0;
  return value;
}

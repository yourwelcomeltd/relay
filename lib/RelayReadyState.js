  ...
  RelayReadyState.prototype.update = function update(nextReadyState, newEvents) {
    ...
    if (prevReadyState.done || prevReadyState.error) {
      if (nextReadyState.stale) {
        ...
        process.env.NODE_ENV !== 'production' ? require('fbjs/lib/warning')(false, 'RelayReadyState: Invalid state change from `%s` to `%s`.', (0, _stringify2['default'])(prevReadyState), (0, _stringify2['default'])(nextReadyState)) : void 0;
      }
      return;
    }
    this._mergeState(nextReadyState, newEvents);
